...
export const readTable: (tableName: string) => Promise<Table> = async (tableName: string) => {
	...
	return new Promise((resolve: (value: Table) => void, reject: (value: Error) => void) => {
		...
		readLbL(`${tableName}.csv`)
			...
			.catch((err) => {
				if (err.code === 'ENOENT')
					reject(`DB ERROR: Table with name "${tableName}" doesn't exist or is not accessible.`);
				reject(err);
			});
	});
};

export const writeTable: (tableValue: Table, overwrite?: boolean) => Promise<string> = async (
	tableValue: Table,
	overwrite?: boolean
) => {
	return new Promise((resolve: (value: string) => void, reject: (value: Error) => void) => {
		...
		writeLbL(`${tableValue.tableName}.csv`, data, overwrite)
			.then((result) => {
				resolve(result);
			})
			.catch((err) => {
