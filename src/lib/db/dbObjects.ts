...
export class Table {
	...
	newRecord(line: string[], needID: boolean = true): Promise<string> {
		return new Promise((resolve: (value: string) => void, reject: (value: Error) => void) => {
			...
			if (this.header.length > line.length && !needID)
				reject(new Error('DB ERROR: Not enough elements'));
			else if (this.header.length < line.length) reject(new Error('DB ERROR: Too many elements'));
			else if (this.header.length > line.length + 1 && needID)
				reject(new Error('DB ERROR: Not enough elements'));
		...
		return new Promise((resolve: (value: string) => void, reject: (value: Error) => void) => {
			...
			this.getRecord(recordID)
				.then((result) => {
					...
					if (Object.keys(newRecord).length > Object.keys(oldRecord).length)
						reject(new Error(`DB ERROR: Too many elements`));
					if (Object.keys(newRecord).length > Object.keys(oldRecord).length)
						reject(new Error(`DB ERROR: Not enough elements`));
