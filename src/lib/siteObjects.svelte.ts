...
export class Product {
	...
	setProps() {
		...
		this.singleOrgPriceM =
			5 * Math.round(((this.singleProductValueM / 100) * (100 + this.organiserProfitMargin)) / 5); // Rounded to the nearest five
		this.allOrgIncomeM = Math.round(this.singleOrgPriceM * this.soldToOrgN);
		this.singlePartPriceM =
			5 * Math.round(((this.singleProductValueM / 100) * (100 + this.participantProfitMargin)) / 5); // Rounded to the nearest five
...
export class Basket {
	...
	addToBasket(prod: Product, price: 'org' | 'part', all: boolean = false): void {
		return new Promise((resolve, reject) => {
			...
			prod.inBasket = true;
			all
				? this.products.push({ prod, price, amt: prod.allRemainingN })
