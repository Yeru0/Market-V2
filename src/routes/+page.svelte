<script lang="ts">
	...
	$effect(() => {
		...
		changeNotes(data.notes[0], basket.payingSum - basket.finalPrice)
			.then((change) => {
		...
		<section class="basket">
			<div class="header">
				...
				<button
					...
					onclick={() => {
						sell(true);
					}}>{basket.products.length == 1 ? 'Termék kivétele' : 'Termékek kivétele'}</button
				>
			</div>
			<form
				onsubmit={() => {
			...
			>
				...
				<div class="notes">
					<div class="paying">
						...
						<div class="line">
							<NoteSelectionTable
								bind:sum={basket.payingSum}
								bind:notes={basket.payingNotes}
								{control}
								{shift}
							></NoteSelectionTable>
						</div>
					...
					<div class="change">
						...
						{#if basket.possibleChange && basket.enoughNotes}
							...
							<div class="line">
								<NoteSelectionTable
									bind:sum={basket.returnSum}
									bind:notes={basket.returnNotes}
									{control}
									{shift}
								></NoteSelectionTable>
							</div>
					...
					<div class="sell-btn">
						<button
							type="submit"
							disabled={5 * Math.round((basket.payingSum - basket.finalPrice) / 5) !==
								basket.returnSum}>Eladás</button
						>
					</div>
				</div>
			</form>
