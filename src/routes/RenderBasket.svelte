<script lang="ts">
	...
	let { basket = $bindable(), control } = $props();
			...
			<tr>
				...
				<td class="button amt">
					<button
						class="remove"
						onclick={() => {
							basket.removeFromBasket(basketProduct.prod, basketProduct.price, false, control);
						}}
						type="button">-</button
					>
					<input
						type="number"
						bind:value={basketProduct.amt}
						min="1"
						max={basketProduct.prod.allRemainingN}
						required
						onchange={() => {
					...
					<button
						class="add"
						onclick={() => {
							basket.addToBasket(basketProduct.prod, basketProduct.price, control);
						}}
				...
				<td class="button action">
					<button
						type="button"
						onclick={() => {
							basket.removeFromBasket(basketProduct.prod, basketProduct.price, true);
						}}>Törlés</button
					></td
				>
			</tr>
