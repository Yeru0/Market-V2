...
<main class="dashboard">
	...
	{#if products.length !== 0}
			...
			<table class="money">
				...
				<tbody>
					...
					<tr>
						...
						<td class="value" class:red={stats.profit < 0} class:green={stats.profit >= 0}
							>{stats.profit} Ft</td
						>
					</tr>
					<tr>
		...
		<section class="notes">
			...
			<div class="table">
				{#if input}
					<NoteChangeTable bind:notes bind:sum={noteSum}></NoteChangeTable>
				{:else}
					<NoteDisplayTable bind:notes bind:sum={noteSum}></NoteDisplayTable>
				{/if}
			</div>
		</section>
					...
					<div class="sell-list">
						{#each events as basket}
							<div class="sell-event">
								<div class="head">
									<h3>
										{basket.events[0].soldTo == 'to'
											? 'Kivett'
											: basket.events[0].soldTo == 'org'
												? 'Szervezőnek eladott'
												: 'Résztvevőnek eladtott'} kosár <em>{basket.events[0].time}</em>-kor.
									</h3>
									<button
										onclick={() => {
								...
								<ol>
									{#each basket.events as event}
										<li>
											<strong>{event.productA.name}</strong> × {event.productB.allRemainingN -
												event.productA.allRemainingN}
										</li>
									{/each}
								</ol>
								{#if basket.overlay}
