	...
	const calculateCols = () => {
		...
		for (const prod of products) {
			if (products.indexOf(prod) <= fitInOneCol) col1.push(prod);
				...
				products.indexOf(prod) > fitInOneCol &&
				products.indexOf(prod) <= 2 * fitInOneCol + 1
			)
				col2.push(prod);
			else if (
				products.indexOf(prod) > 2 * fitInOneCol &&
				products.indexOf(prod) <= 3 * fitInOneCol + 2
			)
				col3.push(prod);
			else col4.push(prod);
		}
		...
		else if (products.length <= 2 * fitInOneCol && products.length > fitInOneCol)
			colClass = 'prices-col2';
		else if (products.length <= 3 * fitInOneCol && products.length > 2 * fitInOneCol)
			colClass = 'prices-col3';
