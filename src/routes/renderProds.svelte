<script lang="ts">
	...
	let { products, basket = $bindable(), control, shift, toast = $bindable() } = $props();
				...
				<tr>
					{#if $priceListStateSellingToOrg}
						<td
							><button
								onclick={() => {
									handleClick('org', product);
								}}
								class:fake-disabled={!product.canAddMore}
								class={product.inBasket ? 'active' : ''}>{product.name}</button
							></td
						>
